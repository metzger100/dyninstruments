...
REPO_NAME="dyninstruments"
PLUGIN_NAME="dyninstruments"
...
VERSION="${DYNINSTRUMENTS_VERSION:-}"
ZIP_SOURCE=""
DATA_DIR=""
TARGET_DIR=""
USE_SYSTEM=0
...
fail() {
  printf 'dyninstruments installer: %s\n' "$*" >&2
  exit 1
}
...
main() {
  ...
  log "dyninstruments source: ${source}"
  log "dyninstruments target: ${target}"
  ...
  log "Installed dyninstruments to ${target}"
  restart_avnav
}
