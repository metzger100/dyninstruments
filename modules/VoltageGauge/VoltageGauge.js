/*!
 * VoltageGauge (UMD) — n-shaped semicircle voltage gauge with warning/alarm sectors (low end / undervoltage)
 *
 * Semantics (DISPLAY units, default "V"):
 *  - Range: minValue .. maxValue
 *  - Alarm zone (matte red):   minValue .. alarmFrom
 *  - Warning zone (matte yellow): alarmFrom .. warningFrom
 *    If only warningFrom exists: minValue .. warningFrom
 *
 ...
 *
 * Defaults (12V lead-acid battery typical):
 *  - minValue: 10
 *  - maxValue: 15
 *  - alarmFrom: 11.6
 *  - warningFrom: 12.2
 *
 * Props:
 *  - value (or voltage): raw voltage
 *  - caption, unit
 *  - minValue, maxValue (DISPLAY units)
 *  - tickMajor, tickMinor (DISPLAY units)
 *  - warningFrom, alarmFrom (DISPLAY units)
 *  - showEndLabels (bool)
 *  - voltageRatioThresholdNormal, voltageRatioThresholdFlat, captionUnitScale
 */
...
(function (root, factory) {
  ...
  function create(def, Helpers) {
    ...
    // --- Voltage formatting (keeps pointer synced with displayed value) ------
    ...
    function formatVoltageString(raw){
      ...
      // Prefer AvNav formatter if available (for consistent rounding/locale)
      if (window.avnav && avnav.api && avnav.api.formatter && typeof avnav.api.formatter.formatDecimal === "function"){
        try {
          // Common AvNav usage in your project: formatDecimal(value, digits, decimals, leadingZero)
    ...
    function displayVoltageFromRaw(raw){
      const s = formatVoltageString(raw);
    ...
    function drawLabelsForMajorValues(ctx, cx, cy, r, family, fontPx, labelInset, minV, maxV, majorStep, arc, showEndLabels){
      ...
      for (let i=0;i<=count;i++){
        ...
        // keep labels clean (integers for 1V ticks; decimals otherwise)
    ...
    function drawValueUnitWithFit(ctx, family, x, y, w, h, value, unit, fit, align){
      ...
      const xStart = (align === 'right') ? (x + w - total) : x;
    ...
    function renderCanvas(canvas, props){
      ...
      // Mode thresholds (owned by VoltageGauge)
      const ratio = W / Math.max(1, H);
      const tN = Number(props.voltageRatioThresholdNormal ?? 1.1);
      const tF = Number(props.voltageRatioThresholdFlat   ?? 3.5);
      ...
      const unit    = (props.unit || 'V').trim();

      const raw = (typeof props.value !== 'undefined') ? props.value : props.voltage;
      const disp = displayVoltageFromRaw(raw);
      const valueText = (disp.text && disp.text.trim()) ? disp.text.trim() : (props.default || '---');
      const valueNum  = isFiniteN(disp.num) ? disp.num : NaN;

      // Range (DISPLAY units) — defaults for 12V battery use case
      let minV = Number(props.minValue ?? 10);
      let maxV = Number(props.maxValue ?? 15);
      ...
      // --- Gauge geometry: maximal + centered --------------------------------
      ...
      // --- LOW-END sectors (undervoltage) ------------------------------------
      // Defaults for lead-acid 12V (resting/under-load varies): these are "reasonable" defaults
      const warningFrom = Number(props.warningFrom ?? 12.2);
      const alarmFrom   = Number(props.alarmFrom   ?? 11.6);

      const aTo = isFinite(alarmFrom) ? Math.max(minV, Math.min(maxV, alarmFrom)) : NaN;
      const wTo = isFinite(warningFrom) ? Math.max(minV, Math.min(maxV, warningFrom)) : NaN;
      ...
      if (mode === 'flat'){
        ...
        if (box.w > 0 && box.h > 0){
          ...
          drawCaptionMax(
            ctx, family,
            topBox.x, topBox.y, topBox.w, topBox.h,
            caption,
          ...
          drawValueUnitWithFit(
            ctx, family,
            bottomBox.x, bottomBox.y, bottomBox.w, bottomBox.h,
            valueText, unit, fit,
            'right'
          );
        }
      ...
      // NORMAL: caption/value/unit inside semicircle
      {
