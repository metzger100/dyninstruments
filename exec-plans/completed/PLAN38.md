...
**Status:** ✅ Implemented | All numeric `*.partN.test.js` fragments were regrouped into topic-named strict tests,
shared setup was extracted once per family, the live `split-spec-fragment` inventory debt was retired, and the complete
quality and coverage gates pass.
...
2. The splits are numeric, not topical. Every `NavMapper.partN.test.js` opens the identical top-level
   `describe("NavMapper", …)` block; the distinguishing content is the `it()` titles inside (the former numeric VMG
   fragment covered VMG + activeRoute, `.part3` covers positions + disconnect, `.part4` covers xteDisplay).
...
### Locked migration convention

- Replace every base file and numeric sibling with `Base.<topic>.test.js`; choose `<topic>` from the behavior covered by
  its contained specs.
- Put setup consumed by more than one topic file in one typed `Base.harness.js` (or a canonical helper) and record that
  non-spec file as `harness-fragment` with a live topic-file parent.
- Remove the old base and numeric-fragment entries from `test-inventory.json`; topic test files are strict by default.
  Do not add entries to, remove entries from, or otherwise change `test-exception-baseline.json`.
...
- `find tests -name '*.part[0-9]*.test.js'` returns zero files.
- `tools/quality-policy/test-inventory.json` contains zero live `split-spec-fragment` entries, while
  `tools/quality-policy/test-exception-baseline.json` remains byte-identical to `HEAD`.
- The migrated and baseline suites both execute 1,899 tests.
- Coverage is identical before and after migration for all 228 production files, including exact line and branch
  covered/total counts.
- `npm run check:all` passes: 437 test files and 1,899 tests pass; coverage is 92.26% statements, 79.77% branches,
  96.83% functions, and 93.24% lines.
- Formatting, linting, strict test typechecking, inventory validation, clone detection, contract tests, smell checks,
  complexity, scaling, documentation, and the 400-line gate all pass.
