...
| TD-019 | Atomicity rollout | Phase 5 is now implemented on `2026-03-10`. `node tools/check-patterns.mjs` reports `5` warnings: `widget-renderer-default-duplication=0`, `engine-layout-default-drift=0`, `canvas-api-typeof-guard=0`, `try-finally-canvas-drawing=0`, `framework-method-typeof-guard=3`, `inline-config-default-duplication=2`. Simplified the shared linear/radial canvas primitive owners to direct `save -> draw -> restore` sequencing, and expanded direct primitive coverage to assert balanced `save`/`restore` behavior on the touched public draw paths. `npm run check:all` passed on March 10, 2026 with `78/78` test files and `474/474` tests green. Promotion criteria: clear the remaining two warn-mode rule backlogs, add any required suppressions for legitimate boundary exceptions, then promote the rules to `block`. Voltage range ownership no longer belongs to `widget-renderer-default-duplication`; it remains separate cleanup work because the wrapper fallback ranges no longer match config-owned editable defaults. | Medium | Medium |
