...
| TD-019 | Atomicity rollout | Phase 0 warn-only atomicity rollout is active. `node tools/check-patterns.mjs` on `2026-03-10` reported `33` warnings: `widget-renderer-default-duplication=18`, `engine-layout-default-drift=6`, `canvas-api-typeof-guard=2`, `try-finally-canvas-drawing=2`, `framework-method-typeof-guard=3`, `inline-config-default-duplication=2`. Promotion criteria: reach `0` warnings for all six rules, add any required suppressions for legitimate boundary exceptions, then promote the rules to `block`. | Medium | Medium |
