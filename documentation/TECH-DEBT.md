...
| TD-023 | Route-points host-action bridge parity | `routePoints` row activation uses normalized payload dispatch (`hostActions.routePoints.activate({ index, pointSnapshot })`) through the temporary bridge (`dyni-workaround(avnav-plugin-actions)`), but full core parity remains bridge-dependent: map recentering only happens if the core relay performs it internally, and double-click `WaypointDialog` opening remains page-local (`EditRoutePage.widgetClick`) and unavailable to plugin renderers. | Medium | Medium |
| TD-024 | Route-points vertical first-pass assumptions | RoutePoints vertical natural height is width-derived and may be finalized at first host commit before surface attach because authoritative shell width is unavailable pre-commit. In hosts where commit state is delayed/missing for a pass, the renderer fails closed to non-vertical assumptions for that pass. | Low | Medium |
...
| TD-009 | 2026-02-20 | Historical: annotated intentional fallback catches in the route-activation pipeline, `runtime/helpers`, `RadialValueMath`, `TemperatureRadialWidget`, `VoltageRadialWidget`, and `PositionCoordinateWidget`; production empty-catch findings cleared. |
...
| TD-008 | 2026-02-20 | Historical: removed direct `avnav.api` access from `SpeedRadialWidget`, `TemperatureRadialWidget`, `VoltageRadialWidget`, `WindRadialWidget`, and `PositionCoordinateWidget`; all formatter access now goes through `componentContext.format.applyFormatter` with mapper-provided formatter props. |
...
| TD-021 | 2026-03-20 | Historical strict compatibility cleanup: removed legacy `[data-dyni]` usage from docs/examples, removed historical ThemeResolver canvas adapters (`resolve(canvas)`, `invalidateCanvas`) in favor of root-only APIs (`resolveForRoot`, `invalidateRoot`, `invalidateAll`), migrated renderer callers to runtime-owned root discovery, and removed non-contract `data-dyni-*` attributes from interactive active-route HTML markup. |
