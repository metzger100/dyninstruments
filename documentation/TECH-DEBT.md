...
| TD-024 | Route-points vertical first-pass assumptions | RoutePoints natural sizing emits no inline shell height or aspect-ratio. Final size is owned by the committed renderer shadow CSS/content after activation. In hosts where commit state is delayed/missing for a pass, the renderer fails closed to non-vertical assumptions for that pass. | Low | Medium |
