...
| Module | File | Owns |
|---|---|---|
| `ValueMath` | `shared/widget-kits/value/ValueMath.js` | finite number conversion, clamping, range normalization, mode thresholds, tick-step profiles, shared formatter text extraction, and angle/direction display formatting |
...
- New `Number(raw)` handling must guard `raw == null` before numeric coercion when missing values should stay missing.
