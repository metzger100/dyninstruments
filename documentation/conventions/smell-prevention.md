...
| Cluster renderer naming drift | `cluster/rendering/` component IDs are cluster-prefixed (e.g. `Vessel*`) | Use role-based renderer IDs in `cluster/rendering/` (e.g. `DateTimeRendererWrapper`) | `check-patterns` (`cluster-renderer-cluster-prefix`) + `check-naming` | block |
