...
- `check-file-size.mjs`: warning at `>250` non-empty lines, failure at `>300` non-empty lines, plus oneliner detection (`--oneliner=warn` default, `--oneliner=block` for strict mode)
...
| Oneliner size-limit bypass | Current warn backlog: 12 dense oneliners + 8 very long packed oneliners (TD-012) | `check-file-size.mjs` (`oneliner` warn mode) | Reformat to multiline; promote to `--oneliner=block` when backlog reaches zero |
