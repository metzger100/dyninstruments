...
- `check-smell-contracts.mjs`: `theme-cache-invalidation`, `dynamic-storekey-clears-on-empty`, `falsy-default-preservation`, `mapper-output-no-nan`, `text-layout-hotspot-budget`, `coordinate-formatter-no-raw-equality-fallback`, `placeholder-contract`, `dash-literal-contract`, `state-screen-precedence-contract`
...
| Copy-paste divergence | Historical: `clamp` implemented in multiple files (`RadialValueMath`, `PositionCoordinateWidget`, `ThreeValueTextWidget`) before TD-006 | `check-patterns.mjs` (`duplicate-functions`, `duplicate-block-clones`) | Use canonical shared implementation (`ValueMath.clamp`) |
...
| Placeholder normalization drift | Formatter output reaches the DOM without `PlaceholderNormalize` | `check-smell-contracts.mjs` (`placeholder-contract`) | Normalize render-model output once at the boundary |
| Dash-literal placeholder drift | Widget source keeps banned literal placeholders instead of routing through the shared helper | `check-smell-contracts.mjs` (`dash-literal-contract`) | Keep placeholder strings centralized in `PlaceholderNormalize` and preserve the RoutePoints carve-out |
| State-screen precedence drift | `pickFirst([...])` call sites are indirect or misordered | `check-smell-contracts.mjs` (`state-screen-precedence-contract`) | Keep state-screen precedence inline and canonical, with AIS's hidden exception only |
