...
- The `canvas-dom` adapter must still implement its own sizing logic (`ResizeObserver` or size signature comparison), because `ResizeFrame` only performs font scaling, not canvas buffer sizing.
