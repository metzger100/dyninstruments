...
(function (root, factory) {
  ...
  function computeMeasurementHints(args) {
    ...
    if (
      ...
      typeof computeResponsiveLineMaxPx !== "function" ||
      typeof clampShare !== "function"
    ) {
    ...
    const flatCenterShare = clampShare(
      Math.max(positionCaptionWidth, coordWidth) /
        Math.max(1, Math.max(positionCaptionWidth, coordWidth) + rowBlockWidth),
      0.28,
      0.56
    );
    ...
    function buildDisplayState(props, math, defaultText) {
      ...
      return {
        ...
        rows: rows.map(function (row) {
          ...
          const courseRawText =
            courseBaseText === defaultText ? courseBaseText : appendUnit(courseBaseText, DEGREE_UNIT, defaultText);
          const distanceRawText =
            distanceBaseText === defaultText ? distanceBaseText : appendUnit(distanceBaseText, row.unit, defaultText);
