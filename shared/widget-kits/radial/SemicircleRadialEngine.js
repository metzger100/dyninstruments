...
(function (root, factory) {
  ...
  function create(def, componentContext) {
    ...
    const majorValueLabels = /** @type {DyniRadialMajorValueLabelsApi} */ (
      componentContext.components.require("RadialMajorValueLabels")
    );
    const text = GU.text;
    ...
    function createRenderer(spec) {
      ...
      return function renderCanvas(canvas, props) {
        ...
        const family = stableDigitsEnabled ? theme.font.familyMono || paint.family : paint.family;
        ...
        const angleNow = Number.isFinite(clampedValue)
          ...
          : NaN;
        const easedAngle = springMotion.resolve(
          /** @type {object} */ (canvas),
          angleNow,
          p.easing !== false,
          Date.now()
        ...
        layerCache.ensureLayer(canvas, staticKey, function (layerCtx, layerName) {
          ...
          majorValueLabels.drawMajorValueLabels({
            ctx: layerCtx,
            family: family,
            geom: layout.geom,
