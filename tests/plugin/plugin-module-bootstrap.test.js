...
function withBootstrapGlobals(overrides, fn) {
  ...
  g.document = overrides.document;
  g.console = overrides.console || console;
  g.AVNAV_BASE_URL = overrides.AVNAV_BASE_URL;
  ...
  return Promise.resolve()
    ...
    .finally(function () {
      ...
      g.AVNAV_BASE_URL = prevAvnavBase;
    });
}
...
/** @param {any} windowRef @param {any} documentRef */
...
describe("plugin.mjs bootstrap", function () {
  ...
  it("stores the provided module API at window.DyniPlugin.avnavApi", async function () {
    ...
    await withBootstrapGlobals(
      ...
      async function () {
        const mod = await importPluginModule();
        await mod.default(api);
        capturedNs = /** @type {any} */ (global).window.DyniPlugin;
      }
    );
  ...
  it("deduplicates same-base module loads and creates distinct script IDs for timestamped base changes", async function () {
    ...
    const bundleScripts = dom.appendedScripts.filter((/** @type {any} */ item) =>
      item.src.endsWith("/bootstrap-bundle.js")
    );
  ...
  it("reloads bootstrap-core across module base generations while deduplicating same-base core loads", async function () {
    const dom = createDomHarness({
      onScriptAppended(node) {
        if (node.src.endsWith("/runtime/plugin-bootstrap-core.js")) {
          /** @type {any} */ (global).window.DyniPluginBootstrapCore = bootstrapCore;
        }
      }
    });
    const runInit = vi.fn(() => Promise.resolve());
    ...
    const coreScripts = dom.appendedScripts.filter((/** @type {any} */ item) =>
  ...
  it("retries bootstrap-core loading after a failed module script request", async function () {
    ...
    const dom = createDomHarness({
      shouldFailScript(node) {
        return node.src.endsWith("/runtime/plugin-bootstrap-core.js") && coreAppendCount === 1;
      },
      onScriptAppended(node) {
        if (node.src.endsWith("/runtime/plugin-bootstrap-core.js")) {
          coreAppendCount += 1;
        ...
        window: {
          ...
          DyniPlugin: {
            ...
            config: {
              ...
              widgetDefinitions: [{ widget: "ClusterWidget", def: { name: "dyni_reload_test" } }]
            }
          }
        }
      },
      async function () {
        const contextWindow = /** @type {any} */ (global).window;
        installInitMocks(contextWindow.DyniPlugin);
        installActualInitRuntime(contextWindow, dom.document);

        const mod = await importPluginModule();
        const firstShutdown = await mod.default(firstApi);
        expect(firstShutdown).toEqual(expect.any(Function));
        firstShutdown();
        expect(contextWindow.DyniPlugin.state).toMatchObject({
          hostActionBridge: null,
          initStarted: false,
          initPromise: null,
          initGenerationId: null
        });
        expect(contextWindow.DyniPlugin.runtime.hostActions).toBeNull();
        expect(contextWindow.DyniPlugin.runtime.componentLoader).toBeNull();
...
describe("plugin-bootstrap-core resolveGlobalRoot", function () {
  it("prefers a global window over globalThis when one is present", async function () {
    await withBootstrapGlobals({ window: { marker: "window" } }, function () {
      expect(bootstrapCore.resolveGlobalRoot()).toBe(global.window);
    });
  });

  it("falls back to globalThis when a declared window global is falsy", function () {
    const g = /** @type {any} */ (global);
    const prevWindow = g.window;
    g.window = null;
    try {
