...
const {
  ...
  function createHarness(options) {
    const opts = options || {};
    const distanceDivisor = Number.isFinite(opts.distanceDivisor)
      ? opts.distanceDivisor
      : 1;
    const calls = {
      staticDraws: [],
      dynamicDraws: [],
    ...
    const theme = {
      ...
      strokeWeight: Object.prototype.hasOwnProperty.call(
        themeOverrides,
        "strokeWeight",
      )
        ? themeOverrides.strokeWeight
        : defaultTheme.strokeWeight,
      pointerDepthWeight: Object.prototype.hasOwnProperty.call(
        themeOverrides,
        "pointerDepthWeight",
      )
    ...
    const realTileLayout = loadFresh(
    ...
    const textTileLayout = {
      ...
      measureMetricTile(args) {
        if (
          typeof (args && args.textFillScale) === "number" &&
          isFinite(args.textFillScale)
        ) {
      ...
      typeof opts.applyFormatter === "function"
        ...
        : function (value, formatterOptions) {
            ...
            if (formatterOptions.formatter === "formatDirection360") {
              ...
              const leading = !!(
                formatterOptions.formatterParameters &&
                formatterOptions.formatterParameters[0]
              );
              return leading
            ...
            create() {
              return {
                ...
                shouldShowWaypoint(mode, layout, showWpName, name, fit) {
                  const result = realPrimitives.shouldShowWaypoint(
                    mode,
                    layout,
                    showWpName,
                    name,
            ...
            create() {
              return {
                ...
                computeMode(W, H, thresholdNormal, thresholdFlat) {
                  const mode = realLayout.computeMode(
                    W,
                    H,
                    thresholdNormal,
                    thresholdFlat,
