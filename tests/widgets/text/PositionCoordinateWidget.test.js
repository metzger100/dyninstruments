...
describe("PositionCoordinateWidget", function () {
  ...
  function makeHelpers(threeRender) {
    return {
      setupCanvas(canvas) {
      ...
      getModule(id) {
        if (id !== "ThreeValueTextWidget") throw new Error("unexpected module: " + id);
        return {
          create() {
            return {
              renderCanvas: threeRender || vi.fn(),
  ...
  it("delegates flat mode rendering to ThreeValueTextWidget", function () {
    const threeRender = vi.fn();
    const spec = loadFresh("widgets/text/PositionCoordinateWidget/PositionCoordinateWidget.js")
      .create({}, makeHelpers(threeRender));
    ...
    expect(threeRender).toHaveBeenCalledWith(canvas, props);
  });
  ...
  it("renders stacked coordinates in normal and high modes", function () {
    ...
    expect(threeRender).not.toHaveBeenCalled();
  });

  it("uses default text for invalid coordinates", function () {
    const spec = loadFresh("widgets/text/PositionCoordinateWidget/PositionCoordinateWidget.js")
      .create({}, makeHelpers(vi.fn()));
