...
describe("CompassRadialWidget", function () {
  ...
  it("eases markerCourse independently from heading and keeps follow-up frames alive while either motion is active", function () {
    const headingMotion = {
      ...
      /** @type {any[]} */
      resolves: [],
      /** @param {any} canvas @param {any} target @param {any} easingEnabled @param {any} nowMs */
    ...
    expect(harness.spec.renderCanvas(canvas, makeCompassProps({ heading: 20, markerCourse: 10 }))).toEqual({
      wantsFollowUpFrame: true
    });
    expect(harness.spec.renderCanvas(canvas, makeCompassProps({ heading: 20, markerCourse: 300 }))).toEqual({
      wantsFollowUpFrame: true
    });
