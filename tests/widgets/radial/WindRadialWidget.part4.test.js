...
describe("WindRadialWidget", function () {
  it("keeps spring state keyed by canvas and snaps immediately when easing is disabled", function () {
    ...
    try {
      ...
      expect(
        harness.spec.renderCanvas(canvasA, makeWindProps({ angle: 12 })),
      ).toBeUndefined();

      nowSpy.mockReturnValue(16);
      expect(
        harness.spec.renderCanvas(canvasA, makeWindProps({ angle: 42 })),
      ).toEqual({ wantsFollowUpFrame: true });

      nowSpy.mockReturnValue(16);
      expect(
        harness.spec.renderCanvas(canvasB, makeWindProps({ angle: 42 })),
      ).toBeUndefined();
      ...
      expect(
        harness.spec.renderCanvas(
          canvasA,
          makeWindProps({ angle: 42, easing: false }),
        ),
      ).toBeUndefined();
    } finally {
      nowSpy.mockRestore();
