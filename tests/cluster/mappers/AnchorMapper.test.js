...
describe("AnchorMapper", function () {
  it("maps distance/watch to formatDistance with per-kind units", function () {
    ...
    expect(
      mapper.translate({ kind: "anchorWatch", watch: 20 }, routeContext("anchorWatch", toolkit)).formatterParameters
    ).toEqual(["m"]);
  });
  ...
  it("uses the shared binding default token when distance selectors are missing", function () {
    ...
    expect(
      mapper.translate({ kind: "anchorDistance", distance: 10 }, routeContext("anchorDistance", customToolkit))
    ).toEqual({
      value: 10,
      caption: "ANCHOR",
      unit: "yd custom",
  ...
  it("maps bearing to direction formatter", function () {
    ...
    const out = mapper.translate(
      { kind: "anchorBearing", bearing: 5, leadingZero: true },
      routeContext("anchorBearing", toolkit)
    );
