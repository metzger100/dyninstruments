...
describe("EnvironmentMapper", function () {
  ...
  it("maps numeric kinds with expected formatters", function () {
    ...
    expect(mapper.translate({ kind: "temp", temp: 20 }, routeContext("temp", makeToolkit())).formatter).toBe(
      "formatTemperature"
    );
    expect(mapper.translate({ kind: "pressure", value: 1013 }, routeContext("pressure", makeToolkit())).formatter).toBe(
      "formatPressure"
    );
    expect(mapper.translate({ kind: "depth", depth: 3 }, routeContext("depth", makeToolkit())).formatter).toBe(
      "formatDistance"
    );
  });
