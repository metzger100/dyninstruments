...
describe("AisTargetTextHtmlWidget", function () {
  ...
  it("updates emitted accent strip width when shell width changes at the same height", function () {
    ...
    const narrowAccent = narrowMounted.mountEl.querySelector(
      ".dyni-ais-target-state-accent",
    );
    const wideAccent = wideMounted.mountEl.querySelector(
      ".dyni-ais-target-state-accent",
    );
    const narrowWidth = readInlinePx(
      narrowAccent && narrowAccent.style ? narrowAccent.style.width : "",
    );
    const wideWidth = readInlinePx(
      wideAccent && wideAccent.style ? wideAccent.style.width : "",
    );
    const narrowRadius = readInlinePx(
      narrowAccent && narrowAccent.style ? narrowAccent.style.borderRadius : "",
    );
    const wideRadius = readInlinePx(
      wideAccent && wideAccent.style ? wideAccent.style.borderRadius : "",
    );
