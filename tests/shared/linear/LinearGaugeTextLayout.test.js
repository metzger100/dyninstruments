...
describe("LinearGaugeTextLayout", function () {
  ...
  it("scales caption, value, and inline fits with textFillScale", function () {
    ...
    expect(compactCalls.captionMax[0]).toBeGreaterThan(
      largeCalls.captionMax[0],
    );
    expect(compactCalls.valueFit[0].vPx).toBeGreaterThan(
      largeCalls.valueFit[0].vPx,
    );
    expect(compactCalls.valueFit[0].uPx).toBeGreaterThan(
      largeCalls.valueFit[0].uPx,
    );
    expect(compactCalls.inlineFit[0].vPx).toBeGreaterThan(
      largeCalls.inlineFit[0].vPx,
    );
    expect(compactCalls.inlineFit[0].cPx).toBeGreaterThan(
      largeCalls.inlineFit[0].cPx,
    );
  });
  ...
  it("uses edge-aware alignment for first and last tick labels", function () {
    ...
    expect(calls.map((entry) => entry.align)).toEqual([
      "left",
      "center",
      "right",
    ]);
  ...
  it("fits all major labels in a cramped graphics-only layout instead of dropping overlaps", function () {
    ...
    const fontPx = Number(
      (fills[0].font.match(/([0-9]+(?:\.[0-9]+)?)px/) || [])[1],
    );

    expect(fills.map((entry) => entry.text)).toEqual([
      "0",
      "10",
      "20",
      "30",
