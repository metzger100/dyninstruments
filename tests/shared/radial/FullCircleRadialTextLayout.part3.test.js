...
describe("FullCircleRadialTextLayout", function () {
  ...
  it("boosts compact text occupancy while keeping layout geometry fixed", function () {
    ...
    expect(compact.calls.threeRows[0].sizes.vPx).toBeGreaterThan(
      large.calls.threeRows[0].sizes.vPx,
    );
    expect(compact.calls.threeRows[0].sizes.cPx).toBeGreaterThan(
      large.calls.threeRows[0].sizes.cPx,
    );
