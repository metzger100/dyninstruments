...
describe("EditRouteHtmlFit", function () {
  ...
  it("uses full value width for ETA/PTS when no unit exists", function () {
    ...
    ["normal", "high"].forEach((mode) => {
      ...
      expect(extractPx(out.metrics.rteEta.valueStyle)).toBeGreaterThan(
        extractPx(out.metrics.dst.valueStyle),
      );
      expect(extractPx(out.metrics.pts.valueStyle)).toBeGreaterThan(
        extractPx(out.metrics.rte.valueStyle),
      );
