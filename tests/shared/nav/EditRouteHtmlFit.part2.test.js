...
describe("EditRouteHtmlFit", function () {
  ...
  it("fits source badge text using its own measurement box", function () {
    ...
    expect(extractPx(longBadge.sourceBadgeStyle)).toBeLessThan(
      extractPx(shortBadge.sourceBadgeStyle),
    );
  });
  ...
  it("shrinks long caption/unit text safely while keeping value fit intact", function () {
    ...
    expect(extractPx(longOut.metrics.dst.labelStyle)).toBeLessThanOrEqual(
      extractPx(shortOut.metrics.dst.labelStyle),
    );
    expect(extractPx(longOut.metrics.dst.unitStyle)).toBeLessThan(
      extractPx(shortOut.metrics.dst.unitStyle),
    );
