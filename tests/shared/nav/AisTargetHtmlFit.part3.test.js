...
describe("AisTargetHtmlFit", function () {
  ...
  it("fits name and status independently in normal, high, and committed vertical modes", function () {
    ...
    expect(extractPx(normalOut.nameStyle)).not.toBe(
      extractPx(normalOut.frontStyle),
    );
    expect(extractPx(highOut.nameStyle)).not.toBe(
      extractPx(highOut.frontStyle),
    );
    expect(extractPx(verticalOut.nameStyle)).not.toBe(
      extractPx(verticalOut.frontStyle),
    );
  });
