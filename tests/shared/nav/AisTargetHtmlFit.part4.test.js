...
describe("AisTargetHtmlFit", function () {
  ...
  it("raises normal/high identity text ceilings above the prior conservative caps", function () {
    ...
    expect(extractPx(normalOut.nameStyle)).toBeGreaterThan(
      Math.floor(normalModel.layout.nameRect.h * 0.62),
    );
    expect(extractPx(normalOut.frontStyle)).toBeGreaterThan(
      Math.floor(normalModel.layout.frontRect.h * 0.72),
    );
    expect(extractPx(highOut.nameStyle)).toBeGreaterThan(
      Math.floor(highModel.layout.nameRect.h * 0.56),
    );
    expect(extractPx(highOut.frontStyle)).toBeGreaterThan(
      Math.floor(highModel.layout.frontRect.h * 0.74),
    );
  });
