...
describe("AisTargetMarkup", function () {
  ...
  it("renders dispatch-mode wrapper, accent, hotspot, and ordered metric tiles", function () {
    ...
    expect(html).not.toContain("dyni-ais-target-metric-brg");
  ...
  it("renders flat BRG data body with front initial and DST/BRG tiles", function () {
    const markup = createMarkup();
    const html = markup.render({
      model: makeModel({
        mode: "flat",
    ...
    expect(html).not.toContain("dyni-ais-target-front\"");
    expect(html).toContain("dyni-ais-target-metric-dst");
    expect(html).toContain("dyni-ais-target-metric-brg");
    expect(html).not.toContain("dyni-ais-target-metric-cpa");
    expect(html).not.toContain("dyni-ais-target-metric-tcpa");
  });
