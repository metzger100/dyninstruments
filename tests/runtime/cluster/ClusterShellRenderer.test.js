...
describe("ClusterShellRenderer", function () {
  ...
  it("normalizes route frames without touching renderers or surfaces", function () {
    ...
    const routeFrame = harness.shellRenderer.normalizeRouteFrame(rawProps, { cluster: "speed" }, harness.context.DyniPlugin.config.clusterRoutes);
    ...
    expect(harness.context.DyniPlugin.runtime.surfaces.createController).not.toHaveBeenCalled();
  });
  ...
  it("renders canvas and html shells with stable mount markup and vertical shell sizing only", function () {
    ...
    const canvasFrame = harness.shellRenderer.normalizeRouteFrame({ kind: "sog", mode: "vertical" }, { cluster: "speed" }, harness.context.DyniPlugin.config.clusterRoutes);
    ...
    const htmlFrame = harness.shellRenderer.normalizeRouteFrame({ cluster: "nav", kind: "activeRoute", mode: "vertical" }, { cluster: "nav" }, harness.context.DyniPlugin.config.clusterRoutes);
    const htmlHtml = harness.shellRenderer.renderRouteShell(
      htmlFrame,
      harness.context.DyniPlugin.config.clusterRoutes.byRouteId["nav/activeRoute"],
      "dyni-host-42",
      { __dyniHostCommitState: { instanceId: "dyni-host-42" } }
    );

    const naturalFrame = harness.shellRenderer.normalizeRouteFrame({ cluster: "nav", kind: "routePoints", mode: "vertical" }, { cluster: "nav" }, harness.context.DyniPlugin.config.clusterRoutes);
    const naturalHtml = harness.shellRenderer.renderRouteShell(
      naturalFrame,
      harness.context.DyniPlugin.config.clusterRoutes.byRouteId["nav/routePoints"],
      "dyni-host-42",
      { __dyniHostCommitState: { instanceId: "dyni-host-42" } }
    );

    expect(canvasHtml).toContain('class="widgetData dyni-shell dyni-surface-canvas dyni-kind-sog dyni-cluster-speed"');
    ...
    expect(htmlHtml).toContain('class="widgetData dyni-shell dyni-surface-html dyni-kind-activeRoute dyni-cluster-nav"');
    ...
    expect(naturalHtml).toContain('class="widgetData dyni-shell dyni-surface-html dyni-kind-routePoints dyni-cluster-nav"');
  ...
  it("keeps ratio shell sizing inline-free when mode is not vertical or absent", function () {
    ...
    const routeMeta = harness.context.DyniPlugin.config.clusterRoutes.byRouteId["speed/sog"];

    const horizontalFrame = harness.shellRenderer.normalizeRouteFrame({ kind: "sog", mode: "horizontal" }, { cluster: "speed" }, harness.context.DyniPlugin.config.clusterRoutes);
    const horizontalHtml = harness.shellRenderer.renderRouteShell(horizontalFrame, routeMeta, "dyni-host-42", {
      __dyniHostCommitState: { instanceId: "dyni-host-42" }
    });

    const absentModeFrame = harness.shellRenderer.normalizeRouteFrame({ kind: "sog" }, { cluster: "speed" }, harness.context.DyniPlugin.config.clusterRoutes);
    const absentModeHtml = harness.shellRenderer.renderRouteShell(absentModeFrame, routeMeta, "dyni-host-42", {
      __dyniHostCommitState: { instanceId: "dyni-host-42" }
    });

    expect(horizontalHtml).toContain('class="widgetData dyni-shell dyni-surface-canvas dyni-kind-sog dyni-cluster-speed"');
    expect(horizontalHtml).toContain('data-dyni-route="speed/sog"');
    expect(horizontalHtml).toContain('data-dyni-surface="canvas-dom"');
    expect(horizontalHtml).toContain('<div class="dyni-surface-canvas"><div class="dyni-surface-canvas-mount"></div></div>');
    ...
    expect(absentModeHtml).toContain('class="widgetData dyni-shell dyni-surface-canvas dyni-kind-sog dyni-cluster-speed"');
    expect(absentModeHtml).toContain('data-dyni-route="speed/sog"');
    expect(absentModeHtml).toContain('data-dyni-surface="canvas-dom"');
    expect(absentModeHtml).toContain('<div class="dyni-surface-canvas"><div class="dyni-surface-canvas-mount"></div></div>');
  ...
  it("returns a diagnostic shell for unknown routes without inline sizing", function () {
    ...
    const routeFrame = harness.shellRenderer.normalizeRouteFrame({ cluster: "nav", kind: "missing" }, { cluster: "nav" }, harness.context.DyniPlugin.config.clusterRoutes);
    const html = harness.shellRenderer.renderRouteShell(routeFrame, null, "dyni-host-42", {
      __dyniHostCommitState: { instanceId: "dyni-host-42" }
    });
