...
describe("ClusterShellRenderer", function () {
  ...
  const ORIGINAL_DYNI_PLUGIN = globalScope.DyniPlugin;

  /** @param {any} clusterRoutes */
  ...
  afterEach(function () {
    if (typeof ORIGINAL_DYNI_PLUGIN === "undefined") {
      delete globalScope.DyniPlugin;
    } else {
      globalScope.DyniPlugin = ORIGINAL_DYNI_PLUGIN;
    }
  });
  ...
  it("normalizes route frames without touching renderers or surfaces", function () {
    ...
    expect(Object.keys(routeFrame)).toEqual(["kind", "mode", "editing", "cluster", "__dyniRouteId", "__dyniRawProps"]);
